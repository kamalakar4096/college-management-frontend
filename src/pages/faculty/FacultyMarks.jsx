...
import {
  Card, CardContent, Typography, Button, MenuItem, TextField,
...
const { data: studentsData } = useApi(
  () => user?.departmentId ? userService.getStudentsByDepartment(user.departmentId) : Promise.resolve([]),
  [user?.departmentId]
)
const students = studentsData?.data || studentsData || []
  ...
  const handleAdd = async () => {
    if (!form.studentId || !form.subjectId || !form.marks || !form.examType) {
  ...
  const columns = [
    ...
    { field: 'examType', headerName: 'Exam Type' },
    {
      ...
      renderCell: ({ value }) => (
        <Typography fontWeight={700} color="#0984e3">{value}/100</Typography>
      )
    },
  ]
  ...
  return (
    <DashboardLayout>
      <PageHeader
        ...
        action={
  <Button type="button" variant="contained" startIcon={<Add />} onClick={() => setOpen(true)}>
    Add Marks
  </Button>
}
      />
        ...
        <Card>
          <CardContent sx={{ p: 2.5 }}>
            <Typography variant="h6" fontWeight={600} mb={2}>
              Marks — {students.find(s => String(s.userId || s.id) === String(viewStudentId))?.name}
            </Typography>
            <DataTable
              columns={columns}
              rows={studentMarks || []}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogTitle>
          <Typography variant="h6" fontWeight={700}>Add Marks</Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth select label="Student *"
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth select label="Exam Type *"
                value={form.examType}
